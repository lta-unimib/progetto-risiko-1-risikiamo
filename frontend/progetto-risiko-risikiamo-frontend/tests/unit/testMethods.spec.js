...
describe('App', () => {
  ...
  test('trade method sets the selectedPaths property to an empty array and update the value of countries', () => {
    ...
    expect(wrapper.vm.selectedPaths).toEqual([{ "attributes": { "title": { "value": "Germany", }, }, }, { "attributes": { "title": { "value": "France", }, }, },]);
  });
  ...
  test('getAdjacentCountries method returns an array of adjacent countries for a given country', () => {
    const wrapper = shallowMount(HomeComponent);
    const adjacentCountries = wrapper.vm.getAdjacentCountries({ "attributes": { "title": { "value": "Germany", }, }, });
  ...
  */
  ...
  test('trades the value between the selected countries and reset the trade', () => {
    ...
    wrapper.setData({ path1: { attributes: { title: { value: 'Germany' } } }, path2: { attributes: { title: { value: 'France' } } }, tradeWindowOpen: true, tradeWindow: { units: 5 } })
  ...
  test('trades the value between the selected countries', () => {
    ...
    wrapper.setData({ path1: { attributes: { title: { value: 'Germany' } } }, path2: { attributes: { title: { value: 'France' } } } })
  ...
  test('should update hoverValue with the passed value', () => {
    const wrapper = shallowMount(HomeComponent)
    wrapper.vm.changeHoverValue({ attributes: { title: { value: 'Germany' } } })
  ...
  test('sets the selectedPaths data property to the provided value', () => {
    const wrapper = shallowMount(HomeComponent)
    wrapper.vm.setSelectedPath('test-path')
